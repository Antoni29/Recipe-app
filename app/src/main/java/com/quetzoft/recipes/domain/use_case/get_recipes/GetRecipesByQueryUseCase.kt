...
class GetRecipesByQueryUseCase @Inject constructor(
    private val repository: RecipeRepository
) {
    operator fun invoke(query: String, offset: Int): Flow<Resource<List<Recipe>>> = flow {
        try {
            emit(Resource.Loading())
            val recipes = repository.getRecipesByQuery(query, offset).toRecipesResponse().results
