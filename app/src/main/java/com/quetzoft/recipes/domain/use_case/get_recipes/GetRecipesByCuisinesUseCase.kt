...
class GetRecipesByCuisineUseCase @Inject constructor(
    ...
    operator fun invoke(cuisine: String, offset: Int): Flow<Resource<List<Recipe>>> = flow {
        try {
            emit(Resource.Loading())
            val recipes = repository.getRecipesByCuisine(cuisine, offset).toRecipesResponse().results
